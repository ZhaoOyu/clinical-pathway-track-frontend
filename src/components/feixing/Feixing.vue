        ...
        <div class="list">
          ...
          >
            ...
            <div class="status overflow">
              <el-tag
                size="mini"
                :type="item.status === '二次入院'?'danger': item.status === '低标入院'?'warning':item.status==='不合理诊疗'?'info':item.status==='超医保支付'?'success':'primary'"
                effect="dark"
              >{{item.status}}</el-tag>
            </div>
          </div>
...
import { feixingData } from '../mock.js'
...
export default {
  ...
  methods: {
    ...
    async getData () {
      this.list = feixingData.slice(0, 10)
    }
  },
  ...
  async mounted () {
    ...
    let timeError = []
    let freeError = []
    let len = feixingData.length
    feixingData.forEach(item => {
      if (item.status === '顺序不当') {
